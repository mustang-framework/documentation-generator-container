...
class RenderTemplatesTask extends Task
{
    ...
    public function __construct()
    {
        ...
        $this->replaceArray = [
            'api.domain.test' => config('mustang.api.url'),
            '{{rate-limit-expires}}' => config('mustang.api.throttle.expires'),
            '{{rate-limit-attempts}}' => config('mustang.api.throttle.attempts'),
            '{{access-token-expires-in}}' => $this->minutesToTimeDisplay(config('mustang.api.expires-in')),
            '{{access-token-expires-in-minutes}}' => config('mustang.api.expires-in'),
            '{{refresh-token-expires-in}}' => $this->minutesToTimeDisplay(config('mustang.api.refresh-expires-in')),
            '{{refresh-token-expires-in-minutes}}' => config('mustang.api.refresh-expires-in'),
